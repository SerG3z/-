/*
MPI-программа генерации множества Мандельброта
Разработать MPI-программу, реализующую параллельный алгоритм построения множества
Мандельброта. В качестве входных данных используются размер области a, b для поиска
точек множества. При проверке принадлежности точки множеству Мандельброта
рекомендуется использовать алгоритм Escape Time. В процессе создания MPI-программы
необходимо учитывать проблему дисбаланса загрузки процессорных ядер и убедиться в
корректности определения времени выполнения параллельной программы.
