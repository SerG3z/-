/*
Потокобезопасный кольцевой буфер на основе блокировок.
Реализовать (в виде класса или в виде набора функций) с использованием блокировок
потокобезопасный кольцевой буфер. Потокобезопасный кольцевой буфер должен 
поддерживать операции добавления и удаления элементов. Необходимо разработать
тестовую программу, в которой создаётся p тестовых параллельных потоков, и каждый
поток выполняет n операций (вид операций выбирается случайным образом) с кольцевым
буфером для случайно сгенерированных элементов.
Необходимо оценить пропускную способность кольцевого буфера.
*/
#include <stdio.h>
#include <stdlib.h>
#include <pthread.h>
